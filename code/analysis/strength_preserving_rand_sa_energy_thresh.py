...
def strength_preserving_rand_sa_energy_thresh(A, rewiring_iter = 10,
    ...
    energy_thresh : float, optional
        Energy threshold. Default = 0.0001.
        Annealing stops when the energy threshold is reached.
    max_stage : int, optional
        Maximum number of annealing stages. Default = 1000.
    niter : int, optional
    ...
    Importantly, the process is stopped when
    the energy threshold is reached.
    ...
    curr_stage = 1
    #annealing while energy is above threshold and max stage is not reached
    while energymin > energy_thresh and curr_stage <= max_stage:
        naccept = 0
        ...
        if verbose:
            print('\nstage {:d}, temp {:.5f}, best energy {:.5f}, '
                  'frac of accepted moves {:.3f}'.format(curr_stage, temp,
                                                         energymin,
                                                         naccept/niter))
        curr_stage += 1
