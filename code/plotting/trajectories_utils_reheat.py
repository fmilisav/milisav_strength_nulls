...
def strength_preserving_rand_sa_reheat_trajectory(A, rewiring_iter = 10,
    ...
    while tracking energy and temperature
    ...
    min_energy_trajectory : list
        Minimum energy at the end of each stage
    ...
    using simulated annealing with a non-monotonic annealing schedule.
    ...
    for istage in tqdm(range(nstage), desc='annealing progress'):
        ...
        if naccept > 0:
            ...
            temps = [reset_temp/2, best_temp]
            temp_arg = np.argmax(temps)
            temp = temps[temp_arg]
            reset_temp = temp
            if verbose:
                if temp_arg == 0:
                    print('Reheating to half the previous reset temperature')
                else:
                    print('Reheating to the best temperature so far')
        if verbose:
        ...
        min_energy_trajectory.append(energymin)
        temperature_trajectory.append(temp)

    return [energy_trajectory, min_energy_trajectory, temperature_trajectory]
